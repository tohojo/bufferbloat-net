...
You will need to set at least one router up as an [AHCP](AHCP.md)
server in order for clients to obtain an ip address on the [babel SSID](Babel_SSID.md). If they are connected via other means (wired, or wireless
...
here's the mesh network at [bloatlab 1](BloatLab_1.md) for example.
