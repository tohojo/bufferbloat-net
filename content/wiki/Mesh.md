...
You will need to set at least one router up as an <link>AHCP</link>
server in order for clients to obtain an ip address on the <link>babel
SSID</link>. If they are connected via other means (wired, or wireless
...
here's the mesh network at <link>bloatlab 1</link> for example.
