...
BK_RATE=`expr $CEIL / 5`   # Min for background
...
tc qdisc add dev $IFACE parent 1:11 handle 110: nfq_codel quantum 500
tc qdisc add dev $IFACE parent 1:12 handle 120: nfq_codel quantum 500
tc qdisc add dev $IFACE parent 1:13 handle 130: nfq_codel quantum 256
...
tc qdisc add dev $DEV parent 1:11 handle 110: nfq_codel
tc qdisc add dev $DEV parent 1:12 handle 120: nfq_codel
tc qdisc add dev $DEV parent 1:13 handle 130: nfq_codel
